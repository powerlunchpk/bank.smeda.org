      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 lg:p-8">
        ...
        >
          ...
          <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-10">
            <div>
              <h2 className="text-3xl font-display font-bold text-slate-900 mb-1">Product Comparison</h2>
              <p className="text-slate-500 font-medium">Side-by-side analysis of your selected banking products</p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="overflow-auto flex-grow p-8">
            <div className="min-w-[800px]">
              <div className="grid grid-cols-4 gap-8 mb-12">
                <div className="pt-20">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-4">Key Features</p>
                </div>
                {products.map((product) => (
                  <div key={product.id} className="text-center group">
                    <div className="w-24 h-24 bg-slate-50 rounded-3xl border border-slate-100 p-4 mx-auto mb-6 flex items-center justify-center shadow-lg shadow-slate-100 group-hover:scale-105 transition-transform duration-500">
                      <Image
                    ...
                    <h3 className="text-lg font-bold text-slate-800 mb-1 line-clamp-1">{product.productName}</h3>
                    <p className="text-sm font-semibold text-smeda-blue">{product.bankName}</p>
                  </div>
                ))}
                ...
                {Array.from({ length: 3 - products.length }).map((_, i) => (
                    <div key={`empty-${i}`} className="opacity-20 flex flex-col items-center">
                        <div className="w-24 h-24 bg-slate-100 rounded-3xl border-2 border-dashed border-slate-300 mx-auto mb-6" />
              ...
              <ComparisonRow label="Loan Range" icon={<DollarSign className="w-4 h-4" />}>
                {products.map(p => (
                  <div key={p.id} className="text-center">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 uppercase">PKR Limit</p>
                  </div>
                ))}
              </ComparisonRow>
              ...
              <ComparisonRow label="Markup Rate" icon={<Percent className="w-4 h-4" />}>
                {products.map(p => (
                  <div key={p.id} className="text-center">
                    <p className="text-lg font-bold text-smeda-blue">{p.markup}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase">Annual rate</p>
                  </div>
                ))}
              </ComparisonRow>
              ...
              <ComparisonRow label="Sectors" icon={<Building2 className="w-4 h-4" />}>
                {products.map(p => (
                  <div key={p.id} className="flex flex-wrap justify-center gap-1.5 px-4">
                    {p.sectors.map(s => (
                      <span key={s} className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded-md text-[10px] font-bold">
                        {s}
                      </span>
                    ))}
                  </div>
              ...
              <ComparisonRow label="Last Updated" icon={<Calendar className="w-4 h-4" />}>
                {products.map(p => (
                  <div key={p.id} className="text-center">
                    <p className="text-sm font-bold text-slate-700">{p.lastUpdated}</p>
                  </div>
                ))}
              </ComparisonRow>

              <div className="grid grid-cols-4 gap-8 mt-12 py-8 bg-slate-50/50 rounded-3xl border border-slate-100">
                <div />
                {products.map(p => (
                  <div key={p.id} className="text-center px-4">
                    <Link 
